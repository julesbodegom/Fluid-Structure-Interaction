%% Input data
N   = 200;      % number of cells
Amp = 0.1;      % amplitude of the initial solution variation
A_m = 0.1;      % amplitude of the mesh motion
u_a = 1;        % advection velocity
tend = 2;       % total simulation time

% Time step refinement parameters
k_vals = 0:5;
dt_array = 0.1 * 2.^(-k_vals);
colors = {'b', 'r', 'g'}; % b=Method 1, r=Method 2, g=Method 3

show_sol = 0; % Keep off for refinement study

% Prepare the log-log plot figure
figure(4);
clf;
hold on;
title('Exercise 2.5: Advecting Flow on Deforming Mesh (BDF2)');
xlabel('\Delta t');
ylabel('Root-Mean-Square Error');
set(gca, 'XScale', 'log', 'YScale', 'log'); 
grid on;

%% Outer loop for the 3 methods
for method = 1:3
    
    err1_array = zeros(1, length(k_vals));
    err2_array = zeros(1, length(k_vals));
    
    %% Inner loop for different time steps
    for k_idx = 1:length(k_vals)
        dt = dt_array(k_idx);
        M = round(tend / dt); % Total number of time steps

        % Initialisation
        dx = zeros(1,N);    % cell volumes
        xi = zeros(1,N+1);  % face centers
        x  = zeros(1,N);    % cell centers

        dx_tn = zeros(1,N);    
        xi_tn = zeros(1,N+1);  

        dx_tnm1 = zeros(1,N);    
        xi_tnm1 = zeros(1,N+1);  

        xi  = (0:N)/N;              
        xi0 = (0:N)/N;              
        x   = (1:N)/N - 0.5/N;      
        dx  = xi(2:N+1) - xi(1:N);  

        % determine the initial condition
        rho0     = 1 + Amp*sin(x*2*pi); 
        rho      = rho0;                
        rho_tn   = rho0;                
        rho_tnm1 = rho0;                

        L = zeros(N);   
        alpha = [1 -1 0]; % First step BE
        sum_sq_err = 0;   % Variable to accumulate error for epsilon_2

        %% Simulation loop
        for t=dt:dt:tend
            dx_tn  = dx;  
            xi_tn  = xi;  
            rho_tn = rho; 
            
            xi     = xi0 + A_m * sin(2*pi*t) * sin(2*pi*xi0); 
            dx     = xi(2:N+1)-xi(1:N);                       
            x      = xi(1:N) + dx/2;                          
            
            dxidt_exnp1   = 2*pi*A_m*cos(2*pi*(t))*sin(2*pi*xi0);         
            dxidt_exnp1_2 = 2*pi*A_m*cos(2*pi*(t-dt/2))*sin(2*pi*xi0);    
            
            %% IMPLEMENTATION OF DGCL
            if (t == dt)
                dxidt_dgcl = (xi - xi_tn) / dt;  
            else
                dxidt_dgcl = (1.5*xi - 2*xi_tn + 0.5*xi_tnm1) / dt;                                         
            end

            if (method == 1)
                dxidt = dxidt_exnp1;
            elseif (method ==2)
                dxidt = dxidt_exnp1_2;
            else
                dxidt = dxidt_dgcl;
            end

            dxidt_r = u_a - dxidt;

            % Internal cells
            for i=2:N-1
                L(i,i-1:i+1) = [0 alpha(1)*dx(i) 0] + dt * 0.5*[-dxidt_r(i) dxidt_r(i+1)-dxidt_r(i) dxidt_r(i+1)];
            end
            % Boundary cells
            L(1,1:2)   = [alpha(1)*dx(1) 0] + dt * 0.5*[dxidt_r(2)-dxidt_r(1) dxidt_r(2)];
            L(1,N)     =                    - dt * 0.5*dxidt_r(1);
            L(N,N-1:N) = [0 alpha(1)*dx(N)] + dt * 0.5*[-dxidt_r(N) dxidt_r(N+1)-dxidt_r(N)];
            L(N,1)     =                    + dt * 0.5*dxidt_r(1);

            % solve system
            rho = (L\(-alpha(2)*(dx_tn.*rho_tn)'-alpha(3)*(dx_tnm1.*rho_tnm1)'))';

            % update old variables
            dx_tnm1  = dx_tn;
            rho_tnm1 = rho_tn;
            xi_tnm1  = xi_tn;
            
            % Use BDF2 after first step
            alpha=[3/2 -2 1/2];
            
            % EXACT SOLUTION for current time 't' and current spatial coordinates 'x'
            rho_ex_t = 1 + Amp * sin(2*pi*(x - u_a*t));
            
            % Accumulate error for epsilon_2 
            sum_sq_err = sum_sq_err + sum((rho - rho_ex_t).^2);
        end

        % Compute final errors for this dt
        err1_array(k_idx) = sqrt( sum((rho - rho_ex_t).^2) / N );
        err2_array(k_idx) = sqrt( sum_sq_err / (N * M) );
    end

    %% Plot the errors for this method
    c = colors{method};
    plot(dt_array, err1_array, [c '-o'], 'DisplayName', ['Method ' num2str(method) ' (\epsilon_1)'], 'LineWidth', 1.5);
    plot(dt_array, err2_array, [c ':s'], 'DisplayName', ['Method ' num2str(method) ' (\epsilon_2)'], 'LineWidth', 1.5);
    
end

legend('show', 'Location', 'best');
hold off;